...
class IPVService : SAPService {
  ...
  Json dashboard(string tenantId) {
    ...
    foreach (sys; systems) {
      if (sys.systemType == "source")
        ++sourceSystems;
      else if (sys.systemType == "target")
        ++targetSystems;
    }
  ...
  private void executeProvisioning(string tenantId, ref IPVJob job) {
    ...
    _store.appendJobLog(createJobLog(tenantId, job.jobId, "info", "job", job.jobId,
        "Read " ~ intToStr(job.usersRead) ~ " users and " ~ intToStr(
        job.groupsRead) ~ " groups from source"));
