...
class ATMServer {
  ...
  private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
    ...
    try {
      ...
      if (segments.length == 6 && segments[3] == "roles" && segments[4] == "technical" && req.method == HTTPMethod
        .PUT) {
        auto context = resolveContext(req, tenantId);
        requirePermission(context, "iam.admin");
      ...
      if (segments.length == 6 && segments[3] == "apps" && segments[5] == "authorize" && req.method == HTTPMethod
        .POST) {
        auto context = resolveContext(req, tenantId);
