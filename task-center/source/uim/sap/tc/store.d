...
class TCStore : SAPStore {
    ...
    Nullable!TCProvider getProvider(string providerId) {
        synchronized (_lock) {
            ...
            if (auto value = key in _providers) return Nullable!TCProvider(*value);
    ...
    TCProvider[] listProviders() {
        TCProvider[] values;
        synchronized (_lock) {
            foreach (_key, value; _providers) values ~= value;
        }
        return values;
    }
    ...
    TCTask upsertTask(TCTask task) {
        synchronized (_lock) {
            ...
            if (auto existing = key in _tasks) {
                ...
                if (task.actionHistory.length == 0) task.actionHistory = existing.actionHistory;
            }
            _tasks[key] = task;
            persistSnapshot();
            return task;
    ...
    Nullable!TCTask getTask(string tenantId, string taskId) {
        synchronized (_lock) {
            ...
            if (auto value = key in _tasks) return Nullable!TCTask(*value);
    ...
    TCTask[] listTasks() {
        TCTask[] values;
        synchronized (_lock) {
            foreach (_key, value; _tasks) values ~= value;
        }
        return values;
    }
    ...
    private void persistSnapshot() {
        try {
            ...
            foreach (_key, provider; _providers) providerValues ~= provider.toJson();
            snapshot["providers"] = providerValues;

            Json taskValues = Json.emptyArray;
            foreach (_key, task; _tasks) taskValues ~= task.toJson();
    ...
    private TCProvider parseProvider(Json item) {
        enforce(item.type == Json.Type.object, new TCStoreException("Provider entry must be an object"));
    ...
    private TCTask parseTask(Json item) {
        enforce(item.type == Json.Type.object, new TCStoreException("Task entry must be an object"));
    ...
    private string readString(Json item, string key, bool required, string fallback = "") {
        if (!(key in item) || item[key].type == Json.Type.null_) {
            if (required) throw new TCStoreException(key ~ " is required in cache item");
            return fallback;
        }
        if (item[key].type != Json.Type.string) throw new TCStoreException(key ~ " must be a string in cache item");
        auto value = item[key].get!string;
        if (required && value.length == 0) throw new TCStoreException(key ~ " cannot be empty in cache item");
    ...
    private bool readBool(Json item, string key, bool fallback) {
        ...
        if (item[key].type != Json.Type.bool_) throw new TCStoreException(key ~ " must be boolean in cache item");
        return item[key].get!bool;
    }

    private SysTime readTime(Json item, string key) {
        if (!(key in item) || item[key].type != Json.Type.string || item[key].get!string.length == 0) {
    ...
    private Json readObject(Json item, string key) {
        ...
        if (item[key].type != Json.Type.object) throw new TCStoreException(key ~ " must be an object in cache item");
        return item[key];
    }

    private string scopedProviderKey(string providerId) {
        return providerId;
    }

    private string scopedTaskKey(string tenantId, string taskId) {
