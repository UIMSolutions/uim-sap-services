...
class CIAService : SAPService {
    ...
    @property const(CIAConfig) config() const { return _config; }
...
HTML";
    ...
    Json listRoles() {
        ...
        foreach (r; _store.listRoles()) arr ~= r.toJson();
        return arr;
    }
    ...
    Json listSystems(string tenantId) {
        Json arr = Json.emptyArray;
        foreach (s; _store.listSystems(tenantId)) arr ~= s.toJson();
        return arr;
    }
    ...
    Json listScenarios() {
        Json arr = Json.emptyArray;
        foreach (s; _store.listScenarios()) arr ~= s.toJson();
        return arr;
    }
    ...
    Json listWorkflows(string tenantId) {
        Json arr = Json.emptyArray;
        foreach (w; _store.listWorkflows(tenantId)) arr ~= w.toJson();
        return arr;
    }
    ...
    Json startWorkflow(string tenantId, string id) {
        ...
        if (wf.status != "planned")
            throw new CIAWorkflowStateException("Workflow must be in 'planned' state to start. Current: " ~ wf.status);
    ...
    Json completeWorkflow(string tenantId, string id) {
        ...
        if (wf.status != "running")
            throw new CIAWorkflowStateException("Workflow must be in 'running' state to complete. Current: " ~ wf.status);
    ...
    Json progressTask(string tenantId, string workflowId, string taskId, Json payload) {
        ...
        static immutable validStatuses = ["pending", "in-progress", "done", "skipped", "failed"];
    ...
    Json listParameters(string tenantId, string workflowId) {
        _requireWorkflow(tenantId, workflowId);
        Json arr = Json.emptyArray;
        foreach (p; _store.listParameters(workflowId)) arr ~= p.toJson();
        return arr;
    }
    ...
    Json listLogs(string tenantId, string workflowId) {
        _requireWorkflow(tenantId, workflowId);
        Json arr = Json.emptyArray;
        foreach (l; _store.listLogs(tenantId, workflowId)) arr ~= l.toJson();
        return arr;
    }
    ...
    private void _seedDefaultData() {
        ...
        sc2.tags        = ["on-prem-to-cloud", "s4hana", "successfactors", "hr-integration"];
        sc2.requiredSystemTypes = ["s4hana-on-prem", "successfactors", "integration-suite"];
        sc2.taskTemplates = [
