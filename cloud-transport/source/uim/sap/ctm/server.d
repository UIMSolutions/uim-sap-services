...
class CTMServer {
  ...
  override void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
    ...
    } catch (CTMNotFoundException e) {
      respondError(res, e.msg, 404);
    } catch (CTMValidationException e) {
      respondError(res, e.msg, 400);
    } catch (CTMAuthorizationException e) {
      respondError(res, e.msg, 403);
    } catch (CTMTransportStateException e) {
  ...
  private void routeRequest(HTTPServerRequest req, HTTPServerResponse res, string subPath) {
    ...
    if (segs.length == 4 && segs[3] == "nodes") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listNodes(tenantId), 200);
        return;
      }
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.createNode(tenantId, req.json), 201);
        return;
      }
    }
    if (segs.length == 5 && segs[3] == "nodes") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.getNode(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "nodes" && segs[5] == "queue") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listQueue(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 4 && segs[3] == "routes") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listRoutes(tenantId), 200);
        return;
      }
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.createRoute(tenantId, req.json), 201);
        return;
      }
    }
    ...
    if (segs.length == 4 && segs[3] == "requests") {
      ...
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.createRequest(tenantId, req.json), 201);
        return;
      }
    }
    if (segs.length == 5 && segs[3] == "requests") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.getRequest(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "requests" && segs[5] == "content") {
      auto reqId = segs[4];
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listContent(tenantId, reqId), 200);
        return;
      }
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.attachContent(tenantId, reqId, req.json), 201);
        return;
      }
    }
