...
class MOBStore : SAPStore {
  ...
  MOBUsageReport appUsageReport(string appId) {
    synchronized (_mutex) {
      MOBUsageReport r = new MOBUsageReport;
      r.appId = appId;
      ...
      foreach (k, v; _users) {
        if (k.length > prefix.length && k[0 .. prefix.length] == prefix) {
          ...
          case MOBConnectionStatus.ACTIVE:
            r.activeUsers++;
            break;
          case MOBConnectionStatus.LOCKED:
            r.lockedUsers++;
            break;
          case MOBConnectionStatus.WIPED:
            r.wipedUsers++;
            break;
          case MOBConnectionStatus.DELETED:
            break;
          }
        }
      }
