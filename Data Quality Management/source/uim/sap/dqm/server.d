...
class DQMServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length == 3 && segments[0] == "v1" && segments[1] == "address" && segments[2] == "cleanse" && req.method == HTTPMethod.POST) {
                res.writeJsonBody(_service.cleanseAddress(req.json), 200);
                return;
            }

            if (segments.length == 2 && segments[0] == "v1" && segments[1] == "geocode" && req.method == HTTPMethod.POST) {
                res.writeJsonBody(_service.geocode(req.json), 200);
                return;
            }

            if (segments.length == 2 && segments[0] == "v1" && segments[1] == "reverse-geocode" && req.method == HTTPMethod.POST) {
                res.writeJsonBody(_service.reverseGeocode(req.json), 200);
                return;
            }

            if (segments.length == 3 && segments[0] == "v1" && segments[1] == "address" && segments[2] == "suggest" && req.method == HTTPMethod.POST) {
