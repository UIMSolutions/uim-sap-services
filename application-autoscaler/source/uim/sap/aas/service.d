...
class AASService : SAPService {
  ...
  Json createPolicy(string appId, Json request) {
    ...
    if (policy.scaleOutThreshold <= policy.scaleInThreshold) {
      throw new AASValidationException(
        "scale_out_threshold must be greater than scale_in_threshold");
    }
    ...
    if (policy.maxInstances > 0 && policy.minInstances > policy.maxInstances) {
      throw new AASValidationException(
        "policy min_instances cannot be greater than policy max_instances");
    }
  ...
  Json evaluate(string appId, Json request, bool applyDecision) {
    ...
    foreach (policy; policies) {
      ...
      auto lowerBound = policy.minInstances > 0 ? max(app.minInstances, policy.minInstances)
        : app.minInstances;
      auto upperBound = policy.maxInstances > 0 ? min(app.maxInstances, policy.maxInstances)
        : app.maxInstances;
      ...
      if (metric >= policy.scaleOutThreshold) {
        ...
        uint candidate = app.currentInstances > policy.scaleInStep
          ? app.currentInstances - policy.scaleInStep : 0;
    ...
    decision.desiredInstances = min(max(decision.desiredInstances, app.minInstances), app
        .maxInstances);
