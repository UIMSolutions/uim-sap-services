...
struct User {
  ...
  override Json toJson() {
    Json json = Json.emptyObject
      .set("schemas", ["urn:ietf:params:scim:schemas:core:2.0:User"].toJson);
    ...
    if (name.formatted.length > 0 || name.givenName.length > 0 || name.familyName.length > 0) {
      ...
      if (name.formatted.length > 0)
        nameJson["formatted"] = name.formatted;
      if (name.givenName.length > 0)
        nameJson["givenName"] = name.givenName;
      if (name.familyName.length > 0)
        nameJson["familyName"] = name.familyName;
      if (name.middleName.length > 0)
        nameJson["middleName"] = name.middleName;
      if (name.honorificPrefix.length > 0)
        nameJson["honorificPrefix"] = name.honorificPrefix;
      if (name.honorificSuffix.length > 0)
        nameJson["honorificSuffix"] = name.honorificSuffix;
      json["name"] = nameJson;
    }
