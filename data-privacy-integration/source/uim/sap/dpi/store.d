...
class DPIStore : SAPStore {
    ...
    DPIRetentionRule[] listRules(string tenantId) {
        ...
        synchronized (_lock) {
            foreach (key, value; _rules) if (belongsTo(key, tenantId)) values ~= value;
        }
        return values;
    }
    ...
    DPIPersonalDataRecord upsertRecord(DPIPersonalDataRecord record) {
        synchronized (_lock) {
            ...
            if (auto existing = key in _records) record.createdAt = existing.createdAt;
            _records[key] = record;
            return record;
        }
    }
    ...
    DPIPersonalDataRecord[] listRecords(string tenantId) {
        ...
        synchronized (_lock) {
            foreach (key, value; _records) {
                if (belongsTo(key, tenantId) && !value.deleted) values ~= value;
            }
        }
        return values;
    }
    ...
    DPIPersonalDataRecord[] listSubjectRecords(string tenantId, string subjectId) {
        ...
        synchronized (_lock) {
            foreach (key, value; _records) {
                if (belongsTo(key, tenantId) && !value.deleted && value.subjectId == subjectId) values ~= value;
            }
        }
        return values;
    }
