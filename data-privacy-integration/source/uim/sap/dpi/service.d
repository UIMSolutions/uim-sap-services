...
class DPIService : SAPService {
    ...
    Json ingestRecord(string tenantId, Json request) {
        ...
        if (record.subjectId.length == 0) throw new DPIValidationException("subject_id is required");
        if (record.category.length == 0) throw new DPIValidationException("category is required");
        if (record.source.length == 0) throw new DPIValidationException("source is required");
    ...
    Json listRetentionRules(string tenantId) {
        ...
        foreach (rule; _store.listRules(tenantId)) resources ~= rule.toJson();
    ...
    Json generateReport(string tenantId, Json request) {
        ...
        DPIPersonalDataRecord[] records = subjectId.length > 0
            ? _store.listSubjectRecords(tenantId, subjectId)
            : _store.listRecords(tenantId);

        Json entries = Json.emptyArray;
        foreach (record; records) entries ~= record.toJson();
    ...
    Json triggerCorrection(string tenantId, Json request) {
        ...
        if (!corrected) throw new DPINotFoundException("Record", recordId);
    ...
    private void validateId(string value, string fieldName) {
        if (value.length == 0) throw new DPIValidationException(fieldName ~ " cannot be empty");
    }
}
