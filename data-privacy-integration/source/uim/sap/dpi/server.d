...
class DPIServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length == 3 && segments[0] == "v1" && segments[1] == "privacy" && segments[2] == "anonymize" && req.method == HTTPMethod.POST) {
            ...
            if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
                ...
                if (segments.length == 5 && segments[3] == "retention" && segments[4] == "rules" && req.method == HTTPMethod.GET) {
                    res.writeJsonBody(_service.listRetentionRules(tenantId), 200);
                    return;
                }

                if (segments.length == 6 && segments[3] == "retention" && segments[4] == "rules" && req.method == HTTPMethod.PUT) {
                    auto ruleId = segments[5];
                    res.writeJsonBody(_service.upsertRetentionRule(tenantId, ruleId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "retention" && segments[4] == "trigger" && req.method == HTTPMethod.POST) {
                    res.writeJsonBody(_service.triggerRetentionDeletion(tenantId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "reporting" && segments[4] == "report" && req.method == HTTPMethod.POST) {
                    res.writeJsonBody(_service.generateReport(tenantId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "reporting" && segments[4] == "export" && req.method == HTTPMethod.POST) {
                    res.writeJsonBody(_service.exportReport(tenantId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "reporting" && segments[4] == "correct" && req.method == HTTPMethod.POST) {
                    res.writeJsonBody(_service.triggerCorrection(tenantId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "reporting" && segments[4] == "delete" && req.method == HTTPMethod.POST) {
    ...
    private void validateAuth(HTTPServerRequest req) {
        ...
        if (!("Authorization" in req.headers)) throw new DPIAuthorizationException("Missing Authorization header");
        auto expected = "Bearer " ~ _service.config.authToken;
        if (req.headers["Authorization"] != expected) throw new DPIAuthorizationException("Invalid token");
    }
