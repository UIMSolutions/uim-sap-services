...
class MDGServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
                ...
                if (segments.length == 5 && segments[3] == "business-partners" && segments[4] == "batch" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 5 && segments[3] == "consolidation" && segments[4] == "ingest" && req.method == HTTPMethod.POST) {
                    res.writeJsonBody(_service.ingestBusinessPartners(tenantId, req.json), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "consolidation" && segments[4] == "duplicates" && req.method == HTTPMethod.GET) {
                    res.writeJsonBody(_service.detectDuplicates(tenantId), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "consolidation" && segments[4] == "merge" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 5 && segments[3] == "quality" && segments[4] == "evaluate" && req.method == HTTPMethod.POST) {
    ...
    private void validateAuth(HTTPServerRequest req) {
        ...
        if (!("Authorization" in req.headers)) {
            throw new MDGAuthorizationException("Missing Authorization header");
        }

        auto expected = "Bearer " ~ _service.config.authToken;
        if (req.headers["Authorization"] != expected) {
            throw new MDGAuthorizationException("Invalid token");
        }
    }
