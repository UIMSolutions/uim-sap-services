...
class MONService : SAPService {
    ...
    Json setAlertEmailChannel(Json request) {
        if (!("recipients" in request) || request["recipients"].type != Json.Type.array) {
    ...
    Json getAlertChannels() {
        Json payload = Json.emptyObject;
        payload["email"] = _store.hasEmailChannel() ? _store.getEmailChannel().toJson() : Json.undefined;
        payload["webhook"] = _store.hasWebhookChannel() ? _store.getWebhookChannel().toJson() : Json.undefined;
        return payload;
    }
    ...
    private bool getBoolean(Json request, string key, bool fallback = false) {
