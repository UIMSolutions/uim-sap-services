...
class CISServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
                ...
                if (segments.length == 5 && segments[3] == "auth" && segments[4] == "login" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 5 && segments[3] == "users" && segments[4] == "invite" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 4 && segments[3] == "delegation-rules" && req.method == HTTPMethod.GET) {
                    res.writeJsonBody(_service.listDelegationRules(tenantId), 200);
                    return;
                }

                if (segments.length == 5 && segments[3] == "delegation-rules" && req.method == HTTPMethod.PUT) {
                ...
                if (segments.length == 5 && segments[3] == "policies" && segments[4] == "authorize" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 5 && segments[3] == "risk" && segments[4] == "evaluate" && req.method == HTTPMethod.POST) {
                ...
                if (segments.length == 5 && segments[3] == "provisioning" && segments[4] == "logs" && req.method == HTTPMethod.GET) {
    ...
    private void validateAuth(HTTPServerRequest req) {
        ...
        if (!("Authorization" in req.headers)) throw new CISAuthorizationException("Missing Authorization header");
        auto expected = "Bearer " ~ _service.config.authToken;
        if (req.headers["Authorization"] != expected) throw new CISAuthorizationException("Invalid token");
    }
