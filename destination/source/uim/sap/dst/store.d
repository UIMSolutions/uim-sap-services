...
class DSTStore : SAPStore {
    ...
    DSTDestination[] listDestinations(string tenantId) {
        ...
        foreach (k, v; _destinations) if (k.startsWith(prefix)) items ~= v;
        return items;
    }
    ...
    DSTDestination[] filterDestinations(string tenantId, string protocol, string proxyType) {
        ...
        foreach (k, v; _destinations) {
            ...
            if (protocol.length > 0 && v.protocol != protocol) continue;
            if (proxyType.length > 0 && v.proxyType != proxyType) continue;
            items ~= v;
        }
        return items;
    }
    ...
    bool tryGetDestination(string tenantId, string name, out DSTDestination dest) {
        ...
        if (k in _destinations) { dest = _destinations[k]; return true; }
        return false;
    }

    bool removeDestination(string tenantId, string name) {
        auto k = key(tenantId, name);
        if (k in _destinations) { _destinations.remove(k); return true; }
        return false;
    }
    ...
    DSTCertificate[] listCertificates(string tenantId) {
        ...
        auto prefix = tp(tenantId);
        foreach (k, v; _certificates) if (k.startsWith(prefix)) items ~= v;
        return items;
    }

    bool tryGetCertificate(string tenantId, string name, out DSTCertificate cert) {
        auto k = key(tenantId, name);
        if (k in _certificates) { cert = _certificates[k]; return true; }
        return false;
    }

    bool removeCertificate(string tenantId, string name) {
        auto k = key(tenantId, name);
        if (k in _certificates) { _certificates.remove(k); return true; }
        return false;
    }
