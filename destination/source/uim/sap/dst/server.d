...
class DSTServer {
    ...
    override void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        catch (DSTNotFoundException e)      { respondError(res, e.msg, 404); }
        catch (DSTValidationException e)    { respondError(res, e.msg, 400); }
        catch (DSTAuthorizationException e) { respondError(res, e.msg, 403); }
        catch (DSTDestinationException e)   { respondError(res, e.msg, 409); }
        catch (DSTException e)              { respondError(res, e.msg, 500); }
    }
    ...
    private void routeRequest(HTTPServerRequest req, HTTPServerResponse res, string subPath) {
        ...
        if (segs.length == 5 && segs[3] == "destinations") {
            auto name = segs[4];
            if (req.method == HTTPMethod.GET)    { res.writeJsonBody(_service.getDestination(tenantId, name), 200); return; }
            if (req.method == HTTPMethod.PUT)    { res.writeJsonBody(_service.updateDestination(tenantId, name, req.json), 200); return; }
            if (req.method == HTTPMethod.DELETE)  { res.writeJsonBody(_service.deleteDestination(tenantId, name), 200); return; }
        }
        ...
        if (segs.length == 4 && segs[3] == "certificates") {
            if (req.method == HTTPMethod.GET)  { res.writeJsonBody(_service.listCertificates(tenantId), 200); return; }
            if (req.method == HTTPMethod.POST) { res.writeJsonBody(_service.createCertificate(tenantId, req.json), 201); return; }
        }

        // ── /certificates/:name ───────────────────────────────────────────
        if (segs.length == 5 && segs[3] == "certificates") {
            auto name = segs[4];
            if (req.method == HTTPMethod.GET)    { res.writeJsonBody(_service.getCertificate(tenantId, name), 200); return; }
            if (req.method == HTTPMethod.DELETE)  { res.writeJsonBody(_service.deleteCertificate(tenantId, name), 200); return; }
        }

        // ── /logs ─────────────────────────────────────────────────────────
        if (segs.length == 4 && segs[3] == "logs") {
            if (req.method == HTTPMethod.GET) { res.writeJsonBody(_service.listLogs(tenantId), 200); return; }
        }
