...
class MDGService {
    ...
    Json mergeDuplicates(string tenantId, Json request) {
        ...
        if (primary.email.length == 0) primary.email = duplicate.email;
        if (primary.phone.length == 0) primary.phone = duplicate.phone;
        if (primary.country.length == 0) primary.country = duplicate.country;
        if (primary.externalId.length == 0) primary.externalId = duplicate.externalId;
    ...
    private bool passesRule(MDGBusinessPartner bp, MDGQualityRule rule) {
        ...
        string value;
        switch (field) {
            case "name": value = bp.name; break;
            case "email": value = bp.email; break;
            case "phone": value = bp.phone; break;
            case "country": value = bp.country; break;
            default: value = ""; break;
        }
