...
class RMSStore {
    ...
    TeamTypeDef[] listTeamTypes(TenantContext tenant) {
        ...
        TeamTypeDef[] list;
        foreach (item; state.teamTypes.byValue) list ~= item;
        return list;
    }
    ...
    FunctionDef[] listFunctions(TenantContext tenant) {
        auto state = stateFor(tenant);
        FunctionDef[] list;
        foreach (item; state.functions.byValue) list ~= item;
        return list;
    }
    ...
    Team[] listTeams(TenantContext tenant) {
        auto state = stateFor(tenant);
        Team[] list;
        foreach (item; state.teams.byValue) list ~= item;
        return list;
    }
    ...
    ResponsibilityRule[] listRules(TenantContext tenant) {
        ...
        foreach (item; state.rules.byValue) list ~= item;
        return list;
    }
    ...
    ResponsibilityRule createRule(TenantContext tenant, Json request) {
        ...
        if (rule.name.length == 0 || rule.contextType.length == 0 || rule.objectType.length == 0) {
            throw new RMSValidationException("rule.name, rule.context_type and rule.object_type are required");
        }
    ...
    Json determineAgents(TenantContext tenant, Json request) {
        ...
        foreach (rule; sorted) {
            ...
            if (rule.contextType != contextType || rule.objectType != objectType) continue;
            ...
            foreach (team; resolvedTeams) {
                ...
                foreach (member; team.members) {
                    if (!memberMatchesRule(member, rule)) continue;
    ...
    Json exportData(TenantContext tenant) {
        ...
        foreach (item; state.teamTypes.byValue) teamTypes ~= item.toJson();
        payload["team_types"] = teamTypes;

        Json functions = Json.emptyArray;
        foreach (item; state.functions.byValue) functions ~= item.toJson();
        payload["functions"] = functions;

        Json teams = Json.emptyArray;
        foreach (item; state.teams.byValue) teams ~= item.toJson();
        payload["teams"] = teams;

        Json rules = Json.emptyArray;
        foreach (item; state.rules.byValue) rules ~= item.toJson();
        payload["rules"] = rules;

        Json logs = Json.emptyArray;
        foreach (item; state.logs) logs ~= item.toJson();
        payload["logs"] = logs;
    ...
    private void fillRule(ref ResponsibilityRule rule, Json request, bool full = true) {
        if (full || "name" in request) rule.name = getString(request, "name", rule.name);
        ...
        try {
            ...
            if ("team_types" in payload && payload["team_types"].type == Json.Type.array) {
                foreach (item; payload["team_types"]) {
                    ...
                    if (def.code.length > 0) state.teamTypes[def.code] = def;
                }
            }
            ...
            if ("functions" in payload && payload["functions"].type == Json.Type.array) {
                foreach (item; payload["functions"]) {
                    ...
                    def.name = getString(item, "name", def.code);
                    def.description = getString(item, "description", "");
                    if (def.code.length > 0) state.functions[def.code] = def;
                }
            }
            ...
            if ("teams" in payload && payload["teams"].type == Json.Type.array) {
                foreach (item; payload["teams"]) {
                    ...
                    if (team.id.length > 0) state.teams[team.id] = team;
                }
            }
            ...
            if ("rules" in payload && payload["rules"].type == Json.Type.array) {
                foreach (item; payload["rules"]) {
                    ...
                    if (rule.id.length > 0) state.rules[rule.id] = rule;
                }
            }
            ...
            if ("logs" in payload && payload["logs"].type == Json.Type.array) {
                foreach (item; payload["logs"]) {
                    ...
                    if (log.id.length > 0) state.logs ~= log;
                }
            }
