...
ISMigration migrationFromJson(string tenantId, Json request) {
    ...
    if ("source_version" in request && request["source_version"].type == Json.Type.string)
        m.sourceVersion = request["source_version"].get!string;
    if ("scenario_type" in request && request["scenario_type"].type == Json.Type.string)
        m.scenarioType = request["scenario_type"].get!string;
    if ("complexity" in request && request["complexity"].type == Json.Type.string)
    ...
    if ("target_runtime" in request && request["target_runtime"].type == Json.Type.string)
