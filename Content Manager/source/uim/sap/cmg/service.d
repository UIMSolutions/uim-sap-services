...
class CMGService {
    ...
    Json integrateProviderContent(string tenantId, string providerId, Json body) {
        validateTenant(tenantId);
        if (providerId.length == 0) throw new CMGValidationException("provider_id is required");

        auto provider = _store.getProvider(tenantId, providerId);
        if (provider.isNull) throw new CMGNotFoundException("Content provider not found");
        if (!provider.get.active) throw new CMGValidationException("Content provider is inactive");
        ...
        if (typesToIntegrate.length == 0) {
            typesToIntegrate = ["apps", "catalogs", "groups", "roles", "shell-plugins"];
        }
    ...
    private void validateTenant(string tenantId) const {
        if (tenantId.length == 0) throw new CMGValidationException("tenant_id is required");
    }

    private string normalizeContentType(string value) const {
        auto normalized = toLower(value);
        if (normalized == "shell_plugins") normalized = "shell-plugins";
        if (normalized != "apps" && normalized != "catalogs" && normalized != "groups" && normalized != "roles" && normalized != "shell-plugins") {
            throw new CMGValidationException("content type must be one of apps|catalogs|groups|roles|shell-plugins");
        }
        return normalized;
    }
    ...
    private string readOptional(Json body, string key, string fallback) const {
        ...
        if (body[key].type != Json.Type.string) throw new CMGValidationException(key ~ " must be a string");
        return body[key].get!string;
    }

    private bool readOptionalBool(Json body, string key, bool fallback) const {
        if (!(key in body) || body[key].type == Json.Type.null_) return fallback;
        if (body[key].type != Json.Type.bool_) throw new CMGValidationException(key ~ " must be a boolean");
    ...
    private string[] readStringArray(Json body, string key) const {
        ...
        if (body[key].type != Json.Type.array) throw new CMGValidationException(key ~ " must be an array");
        foreach (item; body[key]) {
            if (item.type != Json.Type.string) throw new CMGValidationException(key ~ " must contain strings");
    ...
    private Json readObject(Json body, string key) const {
        ...
        if (body[key].type != Json.Type.object) throw new CMGValidationException(key ~ " must be an object");
        return body[key];
    }
}
