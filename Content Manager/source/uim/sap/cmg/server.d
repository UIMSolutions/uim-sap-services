...
class CMGServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
                ...
                if (segments.length == 6 && segments[3] == "providers" && segments[5] == "integrate" && req.method == HTTPMethod.POST) {
    ...
    private void validateAuth(HTTPServerRequest req) {
        ...
        if (!("Authorization" in req.headers)) throw new CMGAuthorizationException("Missing Authorization header");
        auto expected = "Bearer " ~ _service.config.authToken;
        if (req.headers["Authorization"] != expected) throw new CMGAuthorizationException("Invalid token");
    }
