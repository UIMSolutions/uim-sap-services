  ...
  super.handleRequest(req, res);
    ...
    try {
      ...
      if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
        ...
        if (segments.length == 4 && segments[3] == "service-offerings" && req.method == HTTPMethod.GET) {
        ...
        if (segments.length == 6 && segments[3] == "service-instances" && segments[5] == "shares" && req.method == HTTPMethod.POST) {
        ...
        if (segments.length == 5 && segments[3] == "service-bindings" && req.method == HTTPMethod.DELETE) {
          res.writeJsonBody(_service.deleteServiceBinding(tenantId, segments[4]), 200);
          return;
        }

        if (segments.length == 7 && segments[3] == "runtime" && segments[4] == "instances" && segments[6] == "actions" && req.method == HTTPMethod.POST) {
          respondError(res, "Not found", 404);
          return;
        }

        if (segments.length == 8 && segments[3] == "runtime" && segments[4] == "instances" && segments[6] == "actions" && req.method == HTTPMethod.POST) {
