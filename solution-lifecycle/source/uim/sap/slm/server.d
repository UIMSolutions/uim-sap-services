...
class SLMServer {
  ...
  override void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
    ...
    } catch (SLMNotFoundException e) {
      respondError(res, e.msg, 404);
    } catch (SLMValidationException e) {
      respondError(res, e.msg, 400);
    } catch (SLMAuthorizationException e) {
      respondError(res, e.msg, 403);
    } catch (SLMSolutionStateException e) {
  ...
  private void routeRequest(HTTPServerRequest req, HTTPServerResponse res, string subPath) {
    ...
    if (segs.length == 4 && segs[3] == "solutions") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listSolutions(tenantId), 200);
        return;
      }
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.deploySolution(tenantId, req.json), 201);
        return;
      }
    }
    ...
    if (segs.length == 5 && segs[3] == "solutions") {
      auto solId = segs[4];
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.getSolution(tenantId, solId), 200);
        return;
      }
      if (req.method == HTTPMethod.PUT) {
        res.writeJsonBody(_service.updateSolution(tenantId, solId, req.json), 200);
        return;
      }
      if (req.method == HTTPMethod.DELETE) {
        res.writeJsonBody(_service.deleteSolution(tenantId, solId), 200);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "solutions" && segs[5] == "components") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listComponents(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 7 && segs[3] == "solutions" && segs[5] == "components") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.getComponent(tenantId, segs[4], segs[6]), 200);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "solutions" && segs[5] == "deployments") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listDeploymentsForSolution(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "solutions" && segs[5] == "subscriptions") {
      auto solId = segs[4];
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listSubscriptionsForSolution(tenantId, solId), 200);
        return;
      }
      if (req.method == HTTPMethod.POST) {
        res.writeJsonBody(_service.subscribe(tenantId, solId, req.json), 201);
        return;
      }
    }
    ...
    if (segs.length == 6 && segs[3] == "solutions" && segs[5] == "licenses") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listLicensesForSolution(tenantId, segs[4]), 200);
        return;
      }
    }
    ...
    if (segs.length == 4 && segs[3] == "deployments") {
      if (req.method == HTTPMethod.GET) {
        res.writeJsonBody(_service.listDeployments(tenantId), 200);
        return;
      }
    }
