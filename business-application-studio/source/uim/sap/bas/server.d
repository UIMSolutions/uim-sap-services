...
class BASServer {
    ...
    private void handleRequest(HTTPServerRequest req, HTTPServerResponse res) {
        ...
        try {
            ...
            if (segments.length >= 3 && segments[0] == "v1" && segments[1] == "tenants") {
                ...
                if (segments.length == 7 && segments[3] == "workspaces" && segments[5] == "tests" && segments[6] == "local-run" && req.method == HTTPMethod.POST) {
                    auto workspaceId = segments[4];
            ...
            if (segments.length == 3 && segments[0] == "v1" && segments[1] == "platform" && segments[2] == "availability" && req.method == HTTPMethod.GET) {
    ...
    private void validateAuth(HTTPServerRequest req) {
        ...
        if (!("Authorization" in req.headers)) throw new BASAuthorizationException("Missing Authorization header");
        auto expected = "Bearer " ~ _service.config.authToken;
        if (req.headers["Authorization"] != expected) throw new BASAuthorizationException("Invalid token");
    }
